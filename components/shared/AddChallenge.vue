  ...
  >
    ...
    <form @submit.prevent="handleClick" class="py-4">
      ...
      <label class="text-white inline-block mb-2 w-full relative">
        ...
        <h2 @click="selectVisible = !selectVisible" class="text-sm w-full bg-white px-2 py-1 rounded-md placeholder:text-gray text-blakc outline-none"
        >{{ type }}</h2>
        <ul v-if="selectVisible"  class="absolute top-[-100%] w-full bg-white text-blakc rounded-md">
            <li @click="clickSelect('Easy')" class="px-2 py-1 hover:bg-gray">Easy</li>
            <li @click="clickSelect('Medium')" class="px-2 py-1 hover:bg-gray">Medium</li>
            <li @click="clickSelect('Hard')" class="px-2 py-1 hover:bg-gray">Hard</li>
        </ul>
      </label>
      ...
      <div class="grid grid-cols-2 items-center justify-center gap-x-2">
        <button type="submit" class="bg-blue px-2 py-1 text-white text-lg font-medium rounded-lg">Add</button>
        <button @click="$emit('closeModal')" class="bg-white px-2 py-1 text-darkColor text-lg font-medium rounded-lg">Cansel</button>
      </div>
    </form>
  </div>
</template>
